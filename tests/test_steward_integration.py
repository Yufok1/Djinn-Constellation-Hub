...
class TestStewardIntegration(unittest.TestCase):
    ...
    def test_maintenance_task_detection(self):
        ...
        for query in maintenance_queries:
            self.assertTrue(
                hub.is_maintenance_task(query),
                f"Failed to detect maintenance task: {query}",
            )
        ...
        for query in non_maintenance_queries:
            self.assertFalse(
                hub.is_maintenance_task(query),
                f"Incorrectly detected maintenance task: {query}",
            )
    ...
    def test_query_type_classification(self):
        ...
        for query in maintenance_queries:
            ...
            self.assertEqual(
                query_type, "maintenance", f"Failed to classify as maintenance: {query}"
            )
    ...
    def test_agent_keywords_inclusion(self):
        ...
        for keyword in expected_keywords:
            self.assertIn(
                keyword, steward_keywords, f"Missing steward keyword: {keyword}"
            )
